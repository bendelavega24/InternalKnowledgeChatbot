...
def retrieve_docs(question, components, show_scores=False):
    ...
    # multi-query hybrid search -> candidate balance -> rerank larger pool -> balance final docs -> filter -> limit.
    timings = {}
    ...
    balanced_reranked_docs = timed_step(
        ...
        lambda: balance_reranked_documents_with_query_coverage(
            docs=reranked_docs,
            query_doc_groups=hybrid_result.get("query_doc_groups", []),
    ...
    clean_docs = timed_step(
        ...
        lambda: filter_low_quality_docs(balanced_reranked_docs, min_score=MIN_QUALITY_SCORE),
        timings,
    )
